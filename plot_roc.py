...
def verif_pod(y_true, y_pred, threshold):
    # Probability of detection = Hits / (Hits + Misses)
    hits = np.sum(np.logical_and((y_true > threshold), (y_pred > threshold)))
    misses = np.sum(np.logical_and((y_true > threshold), (y_pred < threshold)))
    return hits/(hits+misses)

def verif_far(y_true, y_pred, threshold):
    # False Alarm Rate score = False Alarms / (False Alarms + Hits)
    true_neg = np.sum(np.logical_and((y_true < threshold), (y_pred < threshold)))
    f_alarms = np.sum(np.logical_and((y_true < threshold), (y_pred > threshold)))
